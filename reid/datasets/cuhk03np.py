...
class CUHK03NP(object):
    ...
    def __init__(self, root, split_id=0, num_val=100, cuhk03_labeled=True, verbose=True, **kwargs):
        ...
        num_val_pids = split['num_val_pids']
        num_trainval_pids = split['num_trainval_pids']
        num_query_pids = split['num_query_pids']
        num_gallery_pids = split['num_gallery_pids']
        num_total_pids = num_train_pids + num_query_pids

        num_train_imgs = split['num_train_imgs']
        num_val_imgs = split['num_val_imgs']
        num_trainval_imgs = split['num_trainval_imgs']
        ...
        if verbose:
            ...
            print("  trainval | {:5d} | {:8d}".format(num_trainval_pids, num_trainval_imgs))
        ...
        self.num_trainval_ids = num_trainval_pids
    ...
    def images_dir(self):
        if self.cuhk03_labeled:
            return self.imgs_labeled_dir
        else:
            return self.imgs_detected_dir
    ...
    def _preprocess(self):
        ...
        if not osp.exists(self.imgs_labeled_dir):
            mkdir_if_missing(self.imgs_labeled_dir)
            meta_labeled = _extract_img('labeled')

        if not osp.exists(self.imgs_detected_dir):
    ...
    def get_split(self):
        ...
        def _extract_set(filelist, pids, pid2label, idxs, img_dir, relabel):
            ...
            for idx in idxs:
                ...
                tmp_set.append((img_name, int(pid), camid))
                unique_pids.add(pid)
        ...
        def _extract_new_split(split_dict, img_dir, num_val=100):
            ...
            train_idxs = [idx for idx in trainval_idxs if pids[idx] in train_pids]
            val_idxs = [idx for idx in trainval_idxs if pids[idx] in val_pids]

            pid2label_train = {pid: label for label, pid in enumerate(train_pids)}
            pid2label_val = {pid: label for label, pid in enumerate(val_pids)}
            pid2label = {pid: label for label, pid in enumerate(trainval_pids)}

            query_idxs = split_dict['query_idx'].flatten() - 1
            gallery_idxs = split_dict['gallery_idx'].flatten() - 1
            filelist = split_dict['filelist'].flatten()

            train_info = _extract_set(filelist, pids, pid2label_train, train_idxs, img_dir, relabel=True)
            val_info = _extract_set(filelist, pids, pid2label_val, val_idxs, img_dir, relabel=True)
            trainval_info = _extract_set(filelist, pids, pid2label, trainval_idxs, img_dir, relabel=True)
            ...
            return train_info, val_info, trainval_info, query_info, gallery_info

        if not self.cuhk03_labeled:
            print("Creating new splits for detected images (767/700) ...")
            train_info, val_info, trainval_info, query_info, gallery_info = _extract_new_split(
                loadmat(self.split_new_det_mat_path),
                self.imgs_detected_dir, num_val=self.num_val
            )
            splits = [{
                'train': train_info[0], 'val': val_info[0], 'trainval':trainval_info[0], 'query': query_info[0], 'gallery': gallery_info[0],
                'num_train_pids': train_info[1], 'num_train_imgs': train_info[2],
                'num_val_pids': val_info[1], 'num_val_imgs': val_info[2],
                'num_trainval_pids': trainval_info[1], 'num_trainval_imgs': trainval_info[2],
            ...
            train_info, val_info, trainval_info, query_info, gallery_info = _extract_new_split(
                ...
                self.imgs_labeled_dir, num_val=self.num_val
            )
            splits = [{
                'train': train_info[0], 'val': val_info[0], 'trainval':trainval_info[0], 'query': query_info[0], 'gallery': gallery_info[0],
                'num_train_pids': train_info[1], 'num_train_imgs': train_info[2],
